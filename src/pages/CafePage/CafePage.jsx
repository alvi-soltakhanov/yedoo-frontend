...
const CafePage = () => {
  ...
  const filterfood = foodCafe?.filter(food => {
    return food?.name?.toLowerCase().includes(input.toLowerCase()) || !input
  })

  const category = categories?.find((category)=> category._id === food.categoryId)

  const filteredCategory = category?.filter(category => {
      return category?._id === food?.categoryId
  })
...
//   });
  ...
  return (
    <>
      ...
      <Link to={`/search`}> <p className={style.caves}> ❮ Рестораны</p></Link>
        <div className={style.name}>{cafe1?.name}</div>
        <div className={style.name_discript}>
          {cafe1?.description}
        </div>
        <div className={style.categories}>
          {categories?.map((categories) => {
            return <p key={categories._id}>{categories.name}</p>;
          })}
        </div>
        <div className={style.input}>
            <input type="text" placeholder="Введите название еды.." onChange={(e) => hundleInp(e)} value={input}/>
        </div>
